...
# Data Output
write.table(x = results.GEAS, file = paste0("../output/adipose/baseline/", as.character(x = gsub(pattern = "\\.", replacement = "_", x = args[4])), format(x = Sys.Date(), "_%Y%m%d"), ".txt"), row.names = FALSE, sep = "\t", quote = FALSE)
